...
class QuizCreationViewModel(private val quizRepository: QuizRepository) : ViewModel() {
    ...
    fun submitQuiz(quizName: String, quizCategory: String, hideCreator: Boolean): String {
        ...
        quizRepository.saveQuiz(quiz, key)
        return key
    }
...
class QuizCreationViewModelFactory(private val quizRepository: QuizRepository) :
    ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(QuizCreationViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return QuizCreationViewModel(quizRepository) as T
        }
